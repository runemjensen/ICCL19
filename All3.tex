...
An equality is redundant iff both corresponding inequalities are redundant. If the constraint $c$ is \emph{not} redundant, it is called \emph{non-redundant} or binding.

The feasible area describes the combination of values of the variables in $\VAR(S)$ that satisfy all constraints. However, for some variables $Y\subseteq \VAR(S)$ we are not interested in their values in a feasible point; we just want to know that a satisfying value exists. This information is captured by the \emph{projection} of the feasible area of $S$ w.r.t. $Y$. This is the largest set consisting of values for $\VAR(S)\setminus Y$ that can be extended with values for $Y$ such that all constraints in $S$ are satisfied. 
...
\normalsize{
Assume that we want to eliminate all variables but $u$ (which equals a weighted sum of the rest of the variables).
We then define $S_\trt{g}^0, S_1^0, S_2^0, S_3^0$ and $S_0^4$ as below and project each $S_i^0$ w.r.t. $\{x_i, y_i\}$. We then join these projections together with $S^0_\trt{g}$ in the system $\mathcal{S}$ and project $\mathcal{S}$ w.r.t. $\{z^0_{e,1}, z^0_{e,2}, z^0_{e,3}, z^0_{e,4}, z^0_{c,1}, z^0_{c,2}, z^0_{c,3}, z^0_{c,4}\}$.}
\small{
...
Using our previously described projection method, we obtain the projection of $S$ w.r.t. $Y$ by calling \Call{ProjectNode}{root of $T$}, where $T$ is the tree structure constructed from $S$ and $Y$, and \Call{ProjectNode}{} is described in Algorithm~\ref{alg:decomp}. We note that due to the boundary coarsening (elimination of almost redundant inequalities), this only approximates the projection. However, by setting $\epsilon = 0$, the algorithm indeed returns the correct projection.
...
\begin{table}[tb]
\caption{The size of the VSMs before and after projection into VCMs. }
\label{tab:projections}
\centering
\btablesize
\begin{tabular}{l|r@{ / }r@{ / }l@{ / }r|r@{ / }r@{ / }r@{ / }r}
$\multirow{2}{*}{}$&\multicolumn{4}{c|}{VSM, presolved}& \multicolumn{4}{c}{VCM}\\
							&ineqs&vars	&nzs	&dens&ineqs&vars&nzs&dens\\
\hline
{No weights}	&554	&657	&2784	&5.03&	20 &12	&\phantom{1}155&7.75\\  
{No hydro.} 	&555	&657	&3441	&6.20&	18 &12	&\phantom{1}144&8.00 \\ 
{2 parts} 		&556	&661	&3447	&6.20&	96 &12	&1113&11.59\\ 
{4 parts} 		&564	&671	&3471	&6.15&	64 &12	&\phantom{1}731&11.42\\
{6 parts} 		&570	&679	&3496	&6.13&	80 &12	&\phantom{1}888&11.10\\
{8 parts} 		&576	&685	&3522	&6.11&	52 &12	&\phantom{1}582&11.19\\
%r@{ / }r@{ / }r@{ / }r|
%&\multicolumn{4}{c|}{VSM size}
%&ineqs (eqs)&vars&nzs.& dens.
%&774 (12)&1142&6662&8.61
%&806 (43)&1173&7854&9.74
%&810 (43)&1173&7860&9.70
%&824 (49)&1179&7886&9.57
%&838 (55)&1185&7916&9.44
%&852 (61)&1191&7950&9.33
\bottomrule
...
\begin{table}[tb]
\caption{Projections time using decomposition or not (flat).}
\label{tab:time}
\centering
\btablesize
...
\begin{figure}[tb]
	...
	\caption{Number of inequalities and variables during a flat and a decomposed projection of a multi-commodity flow problem.}
	\label{fig:multicom}
\end{figure}
