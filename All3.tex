...
\newcommand{\mi}{\mathit}
\newcommand{\ve}{\mathbf}
\newcommand{\btablesize}{\begin{footnotesize}}
\newcommand{\etablesize}{\end{footnotesize}}
\newcommand{\set}[2]{\{\;{#1}\;|\;{#2}\;\}}
...
This paper is organized as follows. In Section~\ref{sec:notation} we introduce the required definitions and notation. 
Then Section~\ref{sec:method} outlines the methods used in our FME framework including how block-angular problems are decomposed. Section~\ref{sec:model} shortly present the VSMs
that are projected. Finally follows our experimental results (Section~\ref{sec:results}), before Section~\ref{sec:conclusion} concludes.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
This section introduces our FME-based projection framework. It can be used for massive variable elimination in any linear inequality system, but has been designed to take advantage of block-angular structures often found in real-world models including the VSM. 
%\vspace{-2mm}
...
To detect redundancy, we examine each inequality $c: \ve{a}\cdot \ve{x}\leq b$ in turn and remove it from the system if $\max \ve{a}\cdot \ve{x}$ subject to $S\setminus\{c\}$ is less than or equal to $b$. The property can be checked using an LP solver. Though equalities can be examined this way too, we only examine and remove \underline{in}equalities, since we want to keep the equalities for use in Gauss-elimination. Not all inequalities have to be examined, though. When removing redundancy after projecting $x$ from $S$ we do not need to check inequalities in $\mi{Zero}_S(x)$; if they were non-redundant before the elimination, they will be non-redundant after.
...
\begin{figure}[tb]
	\centering
		\includegraphics[scale=0.9]{figures/almostRedundant2.pdf}
	\caption{The inequality $c$ is almost redundant compared to $c'$, and vice versa.}
	\label{fig:almostRedundant}
\end{figure}
Thus, instead of only collecting a set of redundant inequalities, the manager also collects a set of almost redundant inequalities; the property is checked simultaneously with the ordinary redundancy check. After the parallel redundancy check, \emph{one} thread is then used to go through all the almost redundant inequalities one by one, and the ones that are still almost redundant are removed. %The almost redundant inequalities found this way is then removed from the system $S$.  

We notice that almost redundant inequalities can \emph{not} be removed in parallel. Otherwise, both $c$ and $c'$ in Figure~\ref{fig:almostRedundant} could be found almost redundant by two different threads, causing them both to be eliminated. In e sequential check, however, only the first to be examined would be removed. 
...
Prior to the projection, we perform some simple preprocessing steps repeatedly as long as a reduction happens in a cycle. When performing these steps, we maintain an upper and lower bound for each variable and each constraint's left-hand-side, which initially is set to $+\infty$ and $-\infty$, respectively. The bounds of a constraint is linked to the bounds of the variables it uses, and thus tightening a bound of a variable or constraint can imply a tightening of another bound of a variable or constraint, which we make sure to update as part of the preprocessing.
...
Besides updating bounds, we remove all empty inequalities and unused variables $x\in Y$, and 
%
if the upper and lower bound for a variable equals each other, then we substitute the variable with this bound. 
%
We remove a constraint $\ve{a}\cdot \ve{x} \leq b$ if its upper bound (due to the bounds of the variables) is less or equal to $b$ . 
If the same is true for an equality $\ve{a}\cdot \ve{x}=b$, due to the feasibility assumption, the variables used in $c$ are further substituted with the bounds that makes $\ve{a}\cdot \ve{x}$ 	equal $b$.   

If $\Neg_S(x)=\emptyset$ for an $x\in Y$ (implying that $x$ does not occur in an equality and has no (finite) lower bound) then we remove all constraints in $\Pos_S(x)$. If $\Neg_S(x)$ only consist of the inequality defining the lower bound of $x$, $-x\leq -lb_x$, then we substitute $x$ with $lb_x$ in all constraints. Similarly w.r.t. $\Pos_S(x)$. We notice that this is \emph{not} a normal preprocessing step, but corresponds to making a FME on $x$, and this changes the feasible area of $S$. 

Comparing two constraints syntactically, we can in some cases detect redundancy. To do this, all pairs of constraints are then compared in an efficient manner.
In the following, let $c$ be an inequality or equality $\ve{a}\cdot \ve{x} \otimes b$ (i.e. $\otimes$ is either $=$ or $\leq$), and let $c'$ be $\ve{a}'\cdot \ve{x}\otimes' b'$. 
\begin{itemize} \itemsep0em 
\setcounter{enumi}{\value{counterName}}
\item 
If $\ve{a}=\sigma\cdot \ve{a}'$, then $c$ and is \emph{linearly dependent} of $c'$ (\cite{lassez93}). $c$ is hence redundant if
...
\item Then, we project $S_i^0$ w.r.t. all variables from $Y\cap X_i$, resulting in the system $S'^{\,0}_i$. We do keep the auxiliary $z^0$-variables. 
...
\mathcal{S} = S^0_\trt{g} \cup S'^{\,0}_1\cup \ldots \cup S'^{\,0}_k$. %where $S'^{\,0}_i$ is the projection of $S^0_i$ w.r.t. $Y\cap X_i$ for all $i\in\{1,\ldots, k\}$.
We can then eliminate from $\mathcal{S}$ all the auxiliary $z^0$-variables, $Z^0$, plus any remaining variables in $Y$. 

\paragraph{Example}
As an example, consider the block-structured (in)equality system $S = S_\trt{g} \cup S_1\cup S_2\cup S_3 \cup S_4$, where 
\begin{footnotesize}
\begin{gather*}
...
S_1 \left\{ \begin{array}{ll}	x_1 +   y_1 	&\leq 5\\
																x_1 - 3 y_1 	&\leq 1\\
															4 x_1	+ 2 y_1 	&\leq \shortminus1\end{array}\right.\, 
S_2 \left\{ \begin{array}{ll}	x_2 - 2 y_2 	&\leq 2\\
															 -x_2 -		y_2 	&\leq -2\\ 
																				y_2 	&\leq 4\end{array}\right.\,
S_3 \left\{ \begin{array}{ll}- x_3 +	 	y_3 	&\leq 3\\
														 - 2x_3 - 	y_3 	&\leq \shortminus1\\ 
																x_3 					&\leq 4\end{array}\right.\,
S_4 \left\{ \begin{array}{ll}  x_4 + 2 y_4 	&\leq 8\\
															- x_4 					&\leq \shortminus1\\ 
																		- 	y_4 	&\leq \shortminus1\end{array}\right..
\end{gather*}
\end{footnotesize}
Assume that we want to eliminate all variables but $u$.
We then define $S_\trt{g}^0, S_1^0, S_2^0, S_3^0$ and $S_0^4$ as below and project each $S_i^0$ w.r.t. $\{x_i, y_i\}$. We then join these projections together with $S^0_\trt{g}$ in the system $\mathcal{S}$ and project $\mathcal{S}$ w.r.t. $\set{z^0_{e,i}, z^0_{c,i}}{0\leq i\leq 4}$. %$\{z^0_{e,1}, z^0_{e,2}, z^0_{e,3}, z^0_{e,4}, z^0_{c,1}, z^0_{c,2}, z^0_{c,3}, z^0_{c,4}\}$.}
\begin{footnotesize}
\setlength{\abovedisplayskip}{0pt}
\setlength{\belowdisplayskip}{0pt}
\begin{align*}
S_\trt{g}^0 &= \{-u + z_{e,1}^0 + z_{e,2}^0 + z_{e,3}^0 + z_{e,4}^0 = 0, \: z_{c,1}^0 + z_{c,2}^0 + z_{c,3}^0 + z_{c,4}^0 \leq 16\},\\
S_1^0 			&= S_1\cup \{-z_{e,1}^0 + x_1 + y_1 = 0,\: -z_{c,1}^0 + x_1 + 2 y_1 = 0 \},\\
S_2^0 			&= S_2\cup \{-z_{e,2}^0 + 2 x_1 + 2 y_1 = 0,\: -z_{c,2}^0 + 3x_2 + 4 y_2 = 0 \},\\
S_3^0 			&= S_3\cup \{-z_{e,3}^0 + 3 x_1 + 3 y_1 = 0,\: -z_{c,3}^0 + 5 x_3 + 6y_3 = 0 \},\\
...
\end{proposition}
%
$\mathcal{S}$ has by construction a block-angular structure and instead of eliminating $Z^0\cup Y\setminus(X_1\cup\ldots\cup X_k)$ immediately, if necessary, we can use the same approach as above to postpone ``mixing'' blocks. %further auxiliary variables to postpone ``mixing'' blocks when eliminating the remaining variables. 
To do so, we collect all subsystems into $k_1$ small groups %; we have mostly used groups of size 2, 3 or 1\footnote{``Combining'' a single subsystem corresponds to substituting a variable with a new variable, but can be done for convenience of notation.}. 
and do the following for each group $i$: 
\begin{itemize}\itemsep0em
...
\begin{figure}[tb]
	\centering
		\includegraphics[scale=0.9]{figures/Example4.pdf}
...
S^1_2				&=\{-z^1_{e,2} +z^0_{e,3}+ z^0_{e,4} = 0,\: -z^1_{c,2} +z^0_{c,3}+ z^0_{c,4} = 0\}. 
\end{align*}
}
...
\begin{figure}[tb]
	\centering
		\includegraphics[scale=0.8]{figures/multiflow2.pdf}
